...
const emptyWorkoutRepo = { getHistory: vi.fn().mockResolvedValue([]), getPersonalRecords: vi.fn().mockResolvedValue([]) }
...
const emptyNutritionRepo = { getFoodEntries: vi.fn().mockResolvedValue([]), getGoal: vi.fn().mockResolvedValue(null) }
...
function mockStores() {
  ...
  vi.doMock('@/lib/workoutStore', () => ({ mergeWorkoutFromCloud }))
  vi.doMock('@/lib/progressStore', () => ({ mergeProgressFromCloud }))
  vi.doMock('@/lib/nutritionStore', () => ({ mergeNutritionFromCloud }))
  vi.doMock('@/lib/habitStore', () => ({ mergeHabitFromCloud }))
  vi.doMock('@/lib/gamificationStore', () => ({
    mergeGamificationFromCloud,
    getGamificationState: () => ({ createdAt: '2024-06-01T00:00:00.000Z' }),
  }))

  return { mergeWorkoutFromCloud, mergeProgressFromCloud, mergeNutritionFromCloud, mergeHabitFromCloud, mergeGamificationFromCloud }
}
...
describe('hydrateFromCloud', () => {
  ...
  it('pushes local-only personal records back to the cloud after merging', async () => {
    ...
    vi.doMock('@/lib/progressStore', () => ({
      mergeProgressFromCloud: vi.fn(() => ({ localOnlyWeightLogs: [], localOnlyMeasurements: [], weightGoalToPush: null })),
    }))
    vi.doMock('@/lib/nutritionStore', () => ({ mergeNutritionFromCloud: vi.fn(() => ({ localOnlyEntries: [], goalToPush: null })) }))
    vi.doMock('@/lib/habitStore', () => ({
      mergeHabitFromCloud: vi.fn(() => ({ localOnlyHabits: [], localOnlyEntries: [], localOnlyWaterLogs: [], waterGoalToPush: null })),
    }))
    vi.doMock('@/lib/gamificationStore', () => ({
      mergeGamificationFromCloud: vi.fn(() => ({ localOnlyXpEvents: [], localOnlyEarnedBadges: [], localOnlyChallengeIds: [] })),
      getGamificationState: () => ({ createdAt: '2024-06-01T00:00:00.000Z' }),
    }))
